...
it("POST /generatedata/users?nbUsers", async () => {
  const res = await request(app).post("/generatedata/users")
  .query({
    nbUsers:10
  });
...
it("POST /generatedata/users?nbUser=10&longitude=50.6&latitude=3&longitudeDelta=0.05&latitudeDelta=0.05", async () => {
  ...
  const res = await request(app).post("/generatedata/users").query({
    nbUsers: nbUsers,
    longitude: longitude,
    latitude: latitude,
  ...
  expect(res.body.result).toBe(true);
  expect(res.body.data.length).toBe(nbUsers);
  for (let user of res.body.data) {
    expect(user.currentLocation.coordinates[0]).toBeGreaterThanOrEqual(
      longitude - longitudeDelta / 2
    );
    expect(user.currentLocation.coordinates[0]).toBeLessThanOrEqual(
      longitude + longitudeDelta / 2
    );
    expect(user.currentLocation.coordinates[1]).toBeGreaterThanOrEqual(
      latitude - latitudeDelta / 2
    );
    expect(user.currentLocation.coordinates[1]).toBeLessThanOrEqual(
      latitude + latitudeDelta / 2
    );
  }
  User.deleteMany();
